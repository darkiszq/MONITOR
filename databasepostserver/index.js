...
app.post("/datainsert", async (req, res) => {
    ...
    if (typeof isup === "undefined" || typeof domain === "undefined") {
        return res.status(400).json({ ok: false, error: "Missing isup or domain in request body" });
    }

    isup = isup ? 1 : 0;
    domain = Number(domain);
    if (Number.isNaN(domain)) {
    return res.status(400).json({ ok: false, error: "domain must be a number" });
    }
...
async function idGet(res, domain){
    try{
        let id = -1
        const [result] = await dbPool.execute("SELECT id FROM `domain` where name = ?", [domain])
        console.log(result)
        id = result[0].id
        return id;
    }
...
app.post("/graphfromdomain", async(req,res) => {
    ...
    if(id!=-1){
        try{
            const [result] = await dbPool.execute("SELECT ((SELECT count(*) FROM register where is_up = 1 and domain_id = ?) / ((SELECT count(*) FROM register where is_up = 0 and domain_id = ?) + (SELECT count(*) FROM register where is_up = 1 and domain_id = ?))) as percent FROM `register` WHERE 1 limit 1; ", [id, id, id])
            console.log(result)
...
app.post("/raportfromdomain", async(req, res) =>{
    ...
    if(id!=-1){
        try{
            const [result] = await dbPool.execute("SELECT round(register.time,0) as 'Date', register.is_up as 'Turned on', domain.name as 'Domain' FROM register join domain on domain.id = register.domain_id where register.domain_id = 4 ORDER BY time DESC", [id])
            console.log(result)
